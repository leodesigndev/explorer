...
export function* autoLogin(action) {
  ...
  try {
    const res = yield call(authService.autoLogin, action.payload);
    if (res.error) {
      yield put({
        type: AuthTypes.AUTO_LOGIN_ERROR,
        error: res.message,
      });
    } else {
      history.push("/explorer");
      yield put({ type: AuthTypes.AUTO_LOGIN_SUCCESS, data: res });
    }
  } catch (error) {
    yield put({ type: AuthTypes.AUTO_LOGIN_ERROR, error });
  }
}

/* @TODO handle registration here or on DDM/Monitor ?
...
export default function* allSaga() {
  yield all([
    ...
    takeLatest(AuthTypes.AUTO_LOGIN_REQUEST, autoLogin),
